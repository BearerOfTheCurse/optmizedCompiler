...
const void extractChild(struct Node* source, struct Node* target){ //add source's child to target 

  int num_child = source->get_num_kids();
  for(int i=0;i<num_child;i++){
    target->add_kid(source->get_kid(i));
  }

}

const void makeChild(struct Node* source, struct Node* target){  // add the soucre to target's child if source does not have any child

  int num_child = source->get_num_kids();

  if(num_child == 0 ){
    target->add_kid(source);
    return;
  }
...
struct Node* dealPrimary(struct Node* input){
  int tag = node_get_tag(input);
  if(tag == NODE_TOK_INT_LITERAL || tag == NODE_TOK_IDENT ){
    const char *lexeme = node_get_str(input);
    int ast_tag = tag == NODE_TOK_INT_LITERAL ? AST_INT_LITERAL : AST_VAR_REF;
    struct Node* ast = node_alloc_str_copy(ast_tag,lexeme);
    node_set_source_info(ast,node_get_source_info(input));
    ...
    cerr<<"Error: Invalid primary expression(in ast.cpp)"<<endl;
    exit(-1);
  }
